...
func apiGet(t *testing.T, server *httptest.Server, path, apiKey string) *http.Response {
	...
	// Impede o seguimento automático de redirecionamentos
	client := &http.Client{
...
func apiDelete(t *testing.T, server *httptest.Server, path, apiKey string) *http.Response {
	t.Helper()
...
func apiPatch(t *testing.T, server *httptest.Server, path, body, apiKey string) *http.Response {
	t.Helper()
	req, err := http.NewRequest(http.MethodPatch, server.URL+path, bytes.NewBufferString(body))
	require.NoError(t, err)
	req.Header.Set("Content-Type", "application/json")
	if apiKey != "" {
		req.Header.Set("X-API-Key", apiKey)
...
func TestIntegration_DuplicateURL(t *testing.T) {
	server := buildTestServer(t)
	defer server.Close()

	body := `{"originalUrl":"https://duplicada.example.com"}`
	r1 := apiPost(t, server, "/v1/urls", body, testAPIKey)
	r1.Body.Close()
	require.Equal(t, http.StatusCreated, r1.StatusCode)
	...
	var errResp model.ErrorResponse
	require.NoError(t, json.NewDecoder(r2.Body).Decode(&errResp))
	assert.Equal(t, "URL_ALREADY_EXISTS", errResp.Error.Code)
}

func TestIntegration_InvalidURL_SingleLabelHost(t *testing.T) {
	server := buildTestServer(t)
	defer server.Close()

	body := `{"originalUrl":"https://semPonto"}`
	resp := apiPost(t, server, "/v1/urls", body, testAPIKey)
	defer resp.Body.Close()
	assert.Equal(t, http.StatusBadRequest, resp.StatusCode)
...
func TestIntegration_Delete(t *testing.T) {
	...
	body := `{"originalUrl":"https://delete-int.example.com","customAlias":"del-int"}`
	r1 := apiPost(t, server, "/v1/urls", body, testAPIKey)
	r1.Body.Close()
	require.Equal(t, http.StatusCreated, r1.StatusCode)

	delResp := apiDelete(t, server, "/v1/urls/del-int", testAPIKey)
	delResp.Body.Close()
	assert.Equal(t, http.StatusNoContent, delResp.StatusCode)

	getResp := apiGet(t, server, "/v1/urls/del-int", testAPIKey)
	getResp.Body.Close()
...
func TestIntegration_Delete_NotFound(t *testing.T) {
	...
	resp := apiDelete(t, server, "/v1/urls/naoexiste", testAPIKey)
	defer resp.Body.Close()
	assert.Equal(t, http.StatusNotFound, resp.StatusCode)
}

func TestIntegration_Update_ChangeOriginalURL(t *testing.T) {
	server := buildTestServer(t)
	defer server.Close()

	body := `{"originalUrl":"https://antes.example.com","customAlias":"upd-int"}`
	r1 := apiPost(t, server, "/v1/urls", body, testAPIKey)
	r1.Body.Close()
	require.Equal(t, http.StatusCreated, r1.StatusCode)

	patch := `{"originalUrl":"https://depois.example.com"}`
	patchResp := apiPatch(t, server, "/v1/urls/upd-int", patch, testAPIKey)
	defer patchResp.Body.Close()
	assert.Equal(t, http.StatusOK, patchResp.StatusCode)

	var detail model.URLDetailsResponse
	require.NoError(t, json.NewDecoder(patchResp.Body).Decode(&detail))
	assert.Equal(t, "https://depois.example.com", detail.OriginalURL)
}
