...
func newTestRepo(t *testing.T) repository.URLRepository {
	t.Helper()
	// Usa um banco de dados em memória único por teste para evitar compartilhamento de estado entre testes.
	n := dbCounter.Add(1)
...
func TestFindByOriginalURL(t *testing.T) {
	...
	got, err := repo.FindByOriginalURL(ctx, u.OriginalURL)
	require.NoError(t, err)
	assert.Equal(t, u.ID, got.ID)

	_, err = repo.FindByOriginalURL(ctx, "https://naoexiste.com")
	assert.ErrorIs(t, err, repository.ErrNotFound)
}
